...
| `iql_ab` | CORL IQL adaptive-β protocol |
| `loco4` / `rem7` | suite pack tags |
| `b3` / `b6` / `b10` | CORL `beta` (= fixed & adaptive init) |
| `t0p7` / `t0p5` / `t0p9` | `iql_tau` (expectile) |
...
| `adaptive_beta` | CORL IQL + adaptive β (twin actors) |

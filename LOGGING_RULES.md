...
- IQL-AMO **π_E L_E+L2_RMS** (`execution_meta_loss=le_l2_rms`, family `iql_amo_lel2`)도 본 실험이다. 벨만은 V를 쓰고 평가 정책은 π_E이며, β_E meta-loss만 L_E+L2_RMS이다. 보조 π_B 브랜치는 유지한다. 같은 로스를 다른 family에 붙인 실행은 `execution_meta_lel2` 사유로 ablation이다.

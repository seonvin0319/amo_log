...
from log_layout import normalize, load_config, classify
...
def write_catalog(root, rows):
    ...
    lines=['# 실험 로그','', '| 구분 | 방법 | 환경 | lr | seed | backend | 마지막 평가 step | 로그 |','|---|---|---|---|---:|---|---:|---|']
    for r in rows:
        lr=r.get('meta_lr');lr='—' if lr is None else str(lr)
        lines.append(f"| {r['section']} | {r['method']} | {r['env']} | {lr} | {r['seed']} | {r['backend']} | {r.get('last_eval_step') or '—'} | [{r['run_id']}](../{r['rel_path']}/) |")
...
def build():
    ...
    for section in ('main','ablation'):
        for p in (ROOT/section).rglob('run_meta.json'):
            ...
            c=load_config(p.parent/'config.yaml',m);m.update(classify(m,c))
            m['rel_path']=p.parent.relative_to(ROOT).as_posix();m['last_eval_step']=latest_eval(p.parent)
            m['settings']=c;rows.append(m)
